...
The number of sleep hours per day is a unimodal distribution where mean, median and mode coincide at 7 hours. The distribution is slightly non-symmetrical and cannot be described by the normal curve. The IQR is `r IQR(sleep$sleptim1, na.rm = T)` and the standard deviation is `r round(sd(sleep$sleptim1, na.rm = T), 2)`.
