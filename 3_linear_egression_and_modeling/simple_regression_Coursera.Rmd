...
<li> X The relationship is positive, linear, and very weak. There are no outliers. </li>
...
<ol>
<li> X For each additional home run, the model predicts 1.83 more runs, on average. </li>
...
<li> X The residuals show a curved pattern. </li> 
...
<li> X The residuals are fairly symmetric, with only a slightly longer tail on the right, hence it would be appropriate to deem the the normal distribution of residuals condition met. </li>
</ol>
...
<li> hits </li>
<li> wins </li>
<li> X batting average </li>
</ol>
```{r runs-vs-others}
# type your code for Question 9 here, and Knit
ggplot(data = mlb11, aes(y = runs, x = at_bats)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)
m_at_bats <- lm(runs ~ at_bats, data = mlb11)
summary(m_at_bats) # Multiple R-squared:  0.3729
...
summary(m_hits) # Multiple R-squared:  0.6419

ggplot(data = mlb11, aes(y = runs, x = homeruns)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)

m_homeruns <- lm(runs ~ homeruns, data = mlb11)
summary(m_homeruns) # Multiple R-squared:  0.6266

ggplot(data = mlb11, aes(y = runs, x = bat_avg)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)

m_bat_avg <- lm(runs ~ bat_avg, data = mlb11)
summary(m_bat_avg) # Multiple R-squared:  0.6561

ggplot(data = mlb11, aes(y = runs, x = strikeouts)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)

m_strikeouts <- lm(runs ~ strikeouts, data = mlb11)
summary(m_strikeouts) # Multiple R-squared:  0.1694

ggplot(data = mlb11, aes(y = runs, x = stolen_bases)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)

m_stolen_bases <- lm(runs ~ stolen_bases, data = mlb11)
summary(m_stolen_bases) # Multiple R-squared:  0.002914

ggplot(data = mlb11, aes(y = runs, x = wins)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)

m_wins <- lm(runs ~ wins, data = mlb11)
summary(m_wins) # Multiple R-squared:  0.361
```
...
<ol>
<li> X on-base plus slugging (`new_obs`) </li>
...
ggplot(data = mlb11, aes(y = runs, x = new_onbase)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)

m_onbase <- lm(runs ~ new_onbase, data = mlb11)
summary(m_onbase) # Multiple R-squared:  0.8491

ggplot(data = mlb11, aes(y = runs, x = new_slug)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)

m_slug <- lm(runs ~ new_slug, data = mlb11)
summary(m_slug) # Multiple R-squared:  0.8969

ggplot(data = mlb11, aes(y = runs, x = new_obs)) +
  geom_point() +
  stat_smooth(method = "lm", se = F)

m_obs <- lm(runs ~ new_obs, data = mlb11)
summary(m_obs) # Multiple R-squared:  0.9349
```
