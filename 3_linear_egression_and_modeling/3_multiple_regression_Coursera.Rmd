...
<li> X Yes, revise wording to "Is there an association between beauty and course evaluations?" </li>
...
<ol>
<li> T The median of the distribution is 4.3. </li>
<li> T 25\% of the students gave their professors a score of over 4.6. </li> 
<li> T 11 of students gave a professor a score below 3. </li> 
<li> F The left skewness of the data suggests that the students are less likely to rate the professors highly. </li>
</ol>
```{r score-dist}
...
Variables chosen:
**age (age of professor) and bty_avg (average beauty rating of professor)**
The association between the professor's age and the average beuty rating he/she receives appears to be negative, linear and weak. 

</div>
```{r two-vars-eda}
# type your code for the Exercise here, and Knit
ggplot(evals, aes(x = age, y = bty_avg, color = gender)) +
  geom_point()
```
...
For each additional point in the average beauty rating of the professor, we should expect an increase of 0.07 points on the professor score on average.
...
<li> X Nearly normal residuals: Residuals are right skewed, but the sample size is large, so this may not be an important violation of conditions. </li>
...
# 1. check if explanatory variables are linearly related to the response variable
plot(m_bty_gen$residuals ~ evals$bty_avg) 

# 2. check if residuals are nearly normal, centered around 0
hist(m_bty_gen$residuals) 

qqnorm(m_bty_gen$residuals)
qqline(m_bty_gen$residuals)

# 3. check if residuals have constant variability
plot(m_bty_gen$residuals ~ m_bty_gen$fitted)

plot(abs(m_bty_gen$residuals) ~ m_bty_gen$fitted)

# 4. check for independence of residuals (regarding time they were collected, for instance)
...
Adding **gender** improved the parameter estimation **bty_avg** both in the magnitude of the slope and also on the significance (p-value) of the parameter.
</div>
```{r summary-mlr-model}
# type your code for the Exercise here, and Knit
summary(m_bty_gen)
...
<li> X Tenure Track, Tenured, Teaching </li>
...
The number of professors is the variable with less significance in this model (with a p-value of 0.78).
</div>
...
<ol>
<li> X 0.12 points lower than minority professors, all else held constant. </li>
...
```{r p-val-select}
# type your code for the Exercise here, and Knit
m_full_minus1 <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
summary(m_full_minus1)
```
...
```{r adj-rsq-select}
# type your code for the Exercise here, and Knit
m_test <- lm(score ~ rank + ethnicity + gender + language + age + cls_perc_eval 
             + cls_students + cls_level + cls_credits + bty_avg 
             + pic_outfit + pic_color, data = evals)
...
**Yes, because individual observations can be very influential in linear models.**
</div>
...
**rank = teaching, etnicity = minority, gender = male, language of the school where professor received education = english, young age, higher percentage of class completing the evaluation, teaching in a class with a higher number of students, upperclass, teaching class with more credits, beautiful, picture dressing formal, picture with no color.**
</div>
