...
**2. Distribution shape / skew:** Since the number of atheists or non atheists is larger than 10, the normal condition is also met.
</div>
...
We can be 95% confident that the true proportion of atheists in the US is inside the confidence interval limited by this margin of error.
</div>
...
summary(br12) # with 20 atheists, the sample meets the normal condition
...
summary(jp12) # independence and normal conditions are met
inference(y = response,
          data = jp12,
          statistic = 'proportion',
          success = 'atheist',
...
es %>% 
  ...
  summarise(n = n()) # all inference conditions are met
inference(y = response,
          x = year,
          data = es,
          type = 'ht',
...
us %>% 
  group_by(year, response) %>% 
  summarise(n = n()) # since there are 10 observations in each group we can say all inference conditions are met
inference(y = response,
          x = year,
          data = us,
          type = 'ht',
...
z_star <- qnorm(0.95)
me <- 0.01
p_hat <- 0.5 # to maximize variance
# me = z_star * SE
# SE = sqrt(p_hat * (1 - p_hat) / n)
# me^2 = z_star^2 * p_hat * (1 - p_hat) / n
