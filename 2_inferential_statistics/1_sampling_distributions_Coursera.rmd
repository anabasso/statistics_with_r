...
samp1 %>%
  summarise(x_bar = mean(area), sample_med = median(area), 
            sd = sd(area), sample_iqr = IQR(area),
...
# type your code for the Exercise here, and Run Document
sample_means50 %>%
  summarise(center = mean(x_bar), median = median(x_bar), 
            se = sd(x_bar), iqr = IQR(x_bar),
...
ames %>%
  sample_n(size = 15) %>% 
  summarise(mean_price_estimate = mean(price))
```
...
sample_means_15 <- ames %>%
  rep_sample_n(size = 15,
               replace = T,
               reps = 2000) %>% 
  summarise(x_bar = mean(price))

ggplot(sample_means_15, aes(x = x_bar)) +
  geom_histogram()
```
...
ggplot(sample_means_150, aes(x = x_bar)) +
  geom_histogram()
```
